...
int main (){
    fifo ptr;   //announce stracture
    volatile char result_write;
    volatile int result_read;
    unsigned int i = 100;
    unsigned int j = 0;

    fifo_init(&ptr);      //init fifo information
    fifo_size(&ptr);      //get fifo size information
    //result_write= fifo_putc(&ptr, 19);  // just withdraw one element from FIFO
    while (j< 10){
        result_write= fifo_putc(&ptr, i);
        i++;
        j++;
    }

    fifo_init(&ptr);  // intentonally case EOF
    result_read= fifo_getc(&ptr);
 ...
 *
 */
void fifo_init(fifo *_fifo)  // init fifo array
{
  _fifo->size  = 0;
  _fifo->raddr = 0;
  _fifo->waddr = 0;
